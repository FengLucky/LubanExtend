...
[Pipeline("extend")]
public class ExtendPipeline:IPipeline
{
    ...
    protected void ProcessCodeTarget(string name, ICodeTarget codeTarget)
    {
        ...
        if (EnvManager.Current.TryGetOption(name, "const", false, out var constTargetName))
        {
            s_logger.Info("process const target:{} begin", constTargetName);
            var constTarget = CustomBehaviourManager.Ins.CreateBehaviour<IConstTarget, ConstTargetAttribute>(constTargetName); 
            constTarget.Handle(_genCtx,name,outputManifest);
        }
